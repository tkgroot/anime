...
var anime = (function () {
  ...
  function createTweens(prop, animatable) {
    var previousTween;
  ...
  function createAnimation(animatable, prop) {
    ...
    if (is.num(animType)) {
      var tweens = createTweens(prop, animatable);
    ...
    for (var a = 0, aLength = animatables.length; a < aLength; a++) {
      ...
      if (animatable) {
        for (var p = 0, pLength = properties.length; p < pLength; p++) {
          var animation = createAnimation(animatable, properties[p]); // Make sure the animation is not undefined

...
var anime = (function () {
  ...
  function convertKeyframesToTweens(keyframes, animatable, propertyName, type, index, total) {
    var prevTween;
    var tweens = [];

    var _loop = function _loop(i, l) {
      var keyframe = keyframes[i];
      var tween = convertKeyframeToTween(keyframe, animatable.target, index, total);
      var tweenValue = tween.value;
      var originalValue = decomposeValue(getOriginalAnimatableValue(animatable, propertyName, type));
      var from = void 0,
  ...
  function getAnimations(animatables, keyframes) {
    ...
    for (var i = 0, targetsLength = animatables.length; i < targetsLength; i++) {
      ...
      if (animatable) {
        ...
        for (var j = 0, keysLength = keyframes.length; j < keysLength; j++) {
          ...
          if (is.num(animationType)) {
            var tweens = convertKeyframesToTweens(animationKeyframes, animatable, propertyName, animationType, i, targetsLength);

...
var anime = (function () {
  ...
  var pi = Math.PI;
  var tinyNumber = Number.MIN_VALUE; // Strings
  ...
  function convertValueUnit(el, decomposedValue, unit) {
    ...
    var cached = cache.CSS[valueNumber + unit];
    ...
    var factor = tempEl.offsetWidth ? baseline / tempEl.offsetWidth : 0;
  ...
  function getTransformValue(animatable, propName) {
    ...
    var defaultVal = propName.includes(validTransforms[7]) ? 1 : 0 + getTransformUnit(propName);
  ...
  function recomposeColorValue(tween) {
    var fn = tween.from.numbers;
    var tn = tween.to.numbers;
    var value = rgbaStrings[0];
  ...
  function recomposePathValue(tween) {
    var value = getPathProgress(tween.to.path, tween.progress * tween.to.number) + tween.to.unit;
    if (tween.round) return round(value, tween.round);
    return value;
  }

  function recomposeComplexValue(tween) {
    var value = tween.to.strings[0];

    for (var i = 0, l = tween.to.numbers.length; i < l; i++) {
      var number = getNumberProgress(tween.from.numbers[i], tween.to.numbers[i], tween.progress, tween.round);
      var nextString = tween.to.strings[i + 1];
  ...
  var recomposeValueFunctions = [recomposeNumberValue, recomposeUnitValue, recomposeColorValue, recomposePathValue, recomposeComplexValue];
  ...
  function setTransformsAnimationValue(t, p, v, transforms, manual) {
    ...
    if (p === transforms.last || manual) {
      ...
      transforms.list.forEach(function (value, prop) {
        return transforms.string += "".concat(prop).concat(openParenthesisString).concat(value).concat(closeParenthesisString);
      });
      return t.style.transform = transforms.string;
    }
  }

  var setAnimationValueFunctions = [setObjectAnimationValue, setAttributeAnimationValue, setCssAnimationValue, setTransformsAnimationValue]; // NEEDS TESTING

  function getTargetValue(target, propName, unit) {
    var animatables = getAnimatables(target);

    if (animatables) {
      var animatable = animatables[0];
      var value = getOriginalAnimatableValue(animatable, propName);
      ...
      if (unit) {
        ...
        if (decomposedValue.type === valueTypes.NUMBER || decomposedValue.type === valueTypes.UNIT) {
          var convertedValue = convertValueUnit(animatable.target, decomposedValue, unit);
  ...
  function setTargetsValue(targets, properties) {
    ...
    animatables.forEach(function (animatable) {
      for (var property in properties) {
        ...
        var decomposedValue = decomposeValue(value);
        var originalValue = decomposeValue(getOriginalAnimatableValue(animatable, property, animType));
        ...
        var recomposedValue = recomposeValueFunctions[decomposedValue.type]({
          from: originalValue,
          to: decomposedValue,
          progress: 1
        });
  ...
  function convertKeyframesToTweens(keyframes, animatable, propertyName, animationType) {
    ...
    for (var i = 0, l = keyframes.length; i < l; i++) {
      ...
      var originalDecomposedValue = decomposeValue(getOriginalAnimatableValue(animatable, propertyName, animationType));
      var from = void 0,
      ...
      if (is.arr(tweenValue)) {
        ...
        if (prevTween) {
          ...
          from = _objectSpread2({}, originalDecomposedValue);

          if (is.und(to)) {
            to = _objectSpread2({}, originalDecomposedValue);
          }
        }
      } // Apply operators
      ...
      if (from.type !== to.type) {
        if (from.type === valueTypes.COMPLEX || to.type === valueTypes.COMPLEX) {
          (function () {
            ...
            notComplexValue.numbers = []; // Fallback to 0 for all "from" values

            complexValue.numbers.forEach(function (val, i) {
              return i ? notComplexValue.numbers[i] = 0 : notComplexValue.numbers[i] = notComplexValue.number;
            });
  ...
  function animate() {
    ...
    function setAnimationsProgress(insTime) {
      ...
      while (i < animationsLength) {
        ...
        var animatable = animation.animatable;

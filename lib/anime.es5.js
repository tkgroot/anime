...
var anime = (function () {
  ...
  function rgbToRgba(rgbValue) {
    ...
    var b = +rgba[3];
    var a = +(rgba[4] || 1);
    return ["rgba(".concat(r, ",").concat(g, ",").concat(b, ",").concat(a, ")"), r, g, b, a]; // return `rgba(${r},${g},${b},${a})`;
  ...
  function hslToRgba(hslValue) {
    ...
    return ["rgba(".concat(r, ",").concat(g, ",").concat(b, ",").concat(a, ")"), r, g, b, a]; // return `rgba(${round(r * 255)},${round(g * 255)},${round(b * 255)},${a})`;
  ...
  function getFunctionValue(functionValion, animatable) {
  ...
  function decomposeValue(val, unit) {
    if (is.num(val) && is.und(unit)) {
      ...
      var normalizedColor = normalizeColorToRgba(val);
      return {
        type: valueTypes.COLOR,
        original: normalizedColor[0],
        numbers: [normalizedColor[1], normalizedColor[2], normalizedColor[3], normalizedColor[4]],
        strings: ['rgba(', ',', ',', ',', ')']
      };
    } else {
  ...
  function normalizeTweenValues(tween, animatable) {
    ...
    for (var p in tween) {
      var prop = getFunctionValue(tween[p], animatable);
  ...
  function convertKeyframesToTweens(keyframes, animatable, propertyName) {
    var previousTween;
    return keyframes.map(function (t) {
    ...
    for (var a = 0, aLength = animatables.length; a < aLength; a++) {
      ...
      if (animatable) {
        for (var p = 0, pLength = animationsKeyframes.length; p < pLength; p++) {
          var animation = createAnimation(animatable, animationsKeyframes[p]); // Make sure the animation is not undefined

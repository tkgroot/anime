...
var anime = (function () {
  ...
  function getOriginalTargetValue(target, propName, unit, animatable) {
    var animType = getAnimationType(target, propName);
  ...
  function setTargetsValue(targets, properties) {
    ...
    animatables.forEach(function (animatable) {
      for (var property in properties) {
        ...
        var to = getRelativeValue(replaceValueUnitIfNecessary(value, unit), originalValue);
  ...
  function convertKeyframesToTweens(keyframes, animatable, propertyName) {
    var previousTween;
    var tweens = [];
    ...
    for (var i = 0, l = keyframes.length; i < l; i++) {
      ...
      var toUnit = splitValueUnit(to)[3];
      var originalValue = getOriginalTargetValue(animatable.target, propertyName, toUnit, animatable);
      var previousValue = previousTween ? previousTween.to.original : originalValue;
      var from = is.arr(tweenValue) ? tweenValue[0] : previousValue;
      var fromUnit = splitValueUnit(from)[3] || splitValueUnit(originalValue)[3];
      var unit = toUnit || fromUnit;
      if (is.und(to)) to = previousValue; // console.log(to, ~~to, unit, from, originalValue);
  ...
  function createAnimation(animatable, keyframes) {
    ...
    if (is.num(animType)) {
      var tweens = convertKeyframesToTweens(keyframes, animatable, propertyName);

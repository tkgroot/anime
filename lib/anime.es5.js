...
var anime = (function () {
  ...
  function getAnimationType(el, prop) {
    if (is.obj(el)) {
      ...
      return console.warn("Can't find property '".concat(prop, "' on target '").concat(el, "'."));
    }
  ...
  function createAnimation(animatable, keyframes) {
    var keyframesPropertyName = keyframes[0].propertyName;
    var animationType = getAnimationType(animatable.target, keyframesPropertyName);
    var propertyName = sanitizePropertyName(keyframesPropertyName, animatable.target, animationType);

    if (is.num(animationType)) {
      var tweens = convertKeyframesToTweens(keyframes, animatable, propertyName, animationType);
  ...
  anime.set = function (targets) {
    ...
    var animation = animate(properties); // animation.seek(animation.duration);

...
var anime = (function () {
  ...
  var emptyString = '';
  var pi = Math.PI; // Default animation parameters
  ...
  var validTransforms = ['translateX', 'translateY', 'translateZ', 'rotate', 'rotateX', 'rotateY', 'rotateZ', 'scale', 'scaleX', 'scaleY', 'scaleZ', 'skew', 'skewX', 'skewY', 'perspective', 'matrix', 'matrix3d']; // Regex
  ...
  var relativeValuesExecRgx = /^(\*=|\+=|-=)/; // export const whiteSpaceTestRgx = /\s/g;
  // Colors

  var rgbaStrings = ['rgba(', ',', ',', ',', ')'];
  var hexValuePrefix = '0x';
  ...
  var is = {
    ...
    pth: function pth(a) {
      return is.obj(a) && !is.und(a.totalLength);
    }
  }; // Arrays
  ...
  function convertPxToUnit(el, value, unit) {
    var valueUnit = splitValueUnit(value)[3];
  ...
  function getTransformValue(animatable, propName) {
    var target = animatable.target;
    var defaultVal = stringContains(propName, 'scale') ? 1 : 0 + getTransformUnit(propName);
    var value = getElementTransforms(target).get(propName) || defaultVal;
  ...
  function getFunctionValue(functionValue, animatable) {
  ...
  function getCSSValue(el, prop, unit) {
    var uppercasePropName = prop.replace(lowerCaseRgx, lowerCaseRgxParam).toLowerCase();
  ...
  function getAnimationType(el, prop) {
    if (is.obj(el)) {
      ...
      return console.warn("Can't animate property '".concat(prop, "' on DOM element '").concat(el, "'."));
    }

    return console.warn("Target '".concat(el, "' can't be animated."));
  }
  function getOriginalAnimatableValue(animatable, propName, animationType) {
    var target = animatable.target;
    ...
    switch (animType) {
      ...
      case animationTypes.TRANSFORM:
        return getTransformValue(animatable, propName);

      case animationTypes.CSS:
        return getCSSValue(target, propName);
    }
  }
  function getRelativeValue(from, to) {
    var operator = relativeValuesExecRgx.exec(to);
    if (!operator) return to;
    var t = to.replace(operator[0], emptyString);
    var u = splitValueUnit(t)[3] || 0;
  ...
  function replaceValueUnitIfNecessary(val, unit) {
    if (!unit) {
      ...
      var originalSplitValueUnit = splitValueUnit(val);
  ...
  function decomposeValue(val, unit) {
    var valNumber = +val;
    var valIsNumber = !isNaN(valNumber);
    var hasUnit = !is.und(unit);
    ...
    if (valIsNumber && !hasUnit) {
      ...
      var rgbaNumbers = convertColorStringValuesToRgbaArray(val);
      var rgbaString = rgbaStrings[0] + rgbaNumbers[0] + rgbaStrings[1] + rgbaNumbers[1] + rgbaStrings[2] + rgbaNumbers[2] + rgbaStrings[3] + rgbaNumbers[3] + rgbaStrings[4];
      return {
      ...
      //   console.log(val, unit);
      // }
      var value = replaceValueUnitIfNecessary(is.pth(val) ? val.totalLength : val, unit) + emptyString; // console.log(val);
  ...
  var setValueByType = [function (t, p, v) {
    return t[p] = v;
  }, function (t, p, v) {
    return t.setAttribute(p, v);
  }, function (t, p, v) {
    return t.style[p] = v;
  }, function (t, p, v, transforms, manual) {
  ...
  function getTargetValue(target, propName) {
  ...
  function setTargetsValue(targets, properties) {
    ...
    animatables.forEach(function (animatable) {
      for (var property in properties) {
        ...
        var valueUnit = splitValueUnit(value)[2];
        var originalValue = getOriginalAnimatableValue(animatable, property, animType);
        var unit = valueUnit || splitValueUnit(originalValue)[2];
        var to = getRelativeValue(originalValue, replaceValueUnitIfNecessary(value, unit));
  ...
  function convertKeyframeToTween(keyframe, animatable) {
    ...
    for (var p in keyframe) {
      var prop = getFunctionValue(keyframe[p], animatable);
  ...
  function convertKeyframesToTweens(keyframes, animatable, propertyName, animationType) {
    var previousTween;
    var tweens = [];

    for (var i = 0, l = keyframes.length; i < l; i++) {
      var keyframe = keyframes[i];
      var tween = convertKeyframeToTween(keyframe, animatable);
      var tweenValue = tween.value;
      var isFromToValue = is.arr(tweenValue);
      var originalValue = getOriginalAnimatableValue(animatable, propertyName, animationType);
      var previousValue = previousTween ? previousTween.to.original : originalValue;
      previousTween ? previousTween.to : originalValue;
      var keyTo = isFromToValue ? tweenValue[1] : tweenValue;
      var to = !is.und(keyTo) ? keyTo : previousValue;
      var toComputed = !is.und(keyTo) ? getComputedValue(keyTo) : previousTween ? previousTween.computedTo : getComputedValue(getOriginalAnimatableValue(animatable, propertyName, animationType));
      var toSplitted = splitValueUnit(to);
      toSplitted[2];
      var toUnit = toSplitted[3];
      var from = isFromToValue ? tweenValue[0] : previousValue;
      var fromComputed = isFromToValue ? getComputedValue(tweenValue[0]) : previousTween ? previousTween.computedTo : getComputedValue(getOriginalAnimatableValue(animatable, propertyName, animationType));
      var fromSplitted = splitValueUnit(from);
      fromSplitted[2]; // In case of [from, to] values, checking the original value unit is necessary if no units are provided in the values.

      var fromUnit = fromSplitted[3] || (isFromToValue ? splitValueUnit(previousValue)[3] : undefined);
  ...
  function createAnimation(animatable, keyframes) {
    var propertyName = keyframes[0].propertyName;
    var animType = getAnimationType(animatable.target, propertyName);

    if (is.num(animType)) {
      var tweens = convertKeyframesToTweens(keyframes, animatable, propertyName, animType);
  ...
  function getAnimations(animatables, animationsKeyframes) {
    var animations = [];
    ...
    for (var a = 0, aLength = animatables.length; a < aLength; a++) {
      ...
      if (animatable) {
        for (var p = 0, pLength = animationsKeyframes.length; p < pLength; p++) {
          var animation = createAnimation(animatable, animationsKeyframes[p]);
  ...
  function getTimingsFromAnimations(animations, tweenSettings) {
    ...
    if (!animationsLength) {
      ...
      for (var i = 0; i < animationsLength; i++) {
        ...
        var delay = anim.timelineOffset + anim.delay;

        if (is.und(timings.delay) || delay < timings.delay) {
          timings.delay = delay;
        }

        var duration = anim.timelineOffset + anim.duration;

        if (is.und(timings.duration) || duration > timings.duration) {
          timings.duration = duration;
        }

        var endDelay = anim.timelineOffset + anim.duration - anim.endDelay;
  ...
  function convertPropertyValueToTweens(propertyName, propertyValue, tweenSettings) {
    ...
    settings.propertyName = propertyName; // Override duration if easing is a spring
  ...
  function animate() {
    ...
    function setAnimationsProgress(insTime) {
      ...
      while (i < animationsLength) {
        ...
        for (var j = 0; j < toNumbersLength; j++) {
          var value = void 0;
          var toNumber = tween.to.numbers[j];
          var fromNumber = tween.from.numbers[j] || 0;
  ...
  function stagger(val) {
    ...
    var valueUnit = splitValueUnit(isRange ? val[1] : val);
    var unit = valueUnit[3] ? valueUnit[3] : 0;

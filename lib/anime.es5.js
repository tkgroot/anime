...
var anime = (function () {
  ...
  function setTransformsAnimationValue(t, p, v, needsRender) {
    var cached = cache.DOM.get(t);
    cached.transforms[p] = v;

    if (needsRender) {
      cached.transformString = emptyString;

      for (var prop in cached.transforms) {
        cached.transformString += "".concat(prop).concat(openParenthesisString).concat(cached.transforms[prop]).concat(closeParenthesisString);
      }
    ...
    for (var i = 0, targetsLength = targets.length; i < targetsLength; i++) {
      ...
      if (target) {
        ...
        for (var j = 0, keysLength = keyframes.length; j < keysLength; j++) {
          ...
          var propertyName = sanitizePropertyName(animationKeyframesPropertyName, target, animationType);

          if (is.num(animationType)) {
            var tweens = convertKeyframesToTweens(animationKeyframes, target, propertyName, animationType, i, targetsLength);

...
!(function (t, e) {
  ...
  function c(t, e) {
    ...
    return (
      s >= 0 &&
      h >= 0 && { top: n, bottom: i, left: o, right: r, width: s, height: h }
    );
  }
  function u(t) {
    var e;
  ...
  (i.prototype.THROTTLE_TIMEOUT = 100),
    ...
    (i.prototype._parseRootMargin = function (t) {
      ...
      return (
        (n[1] = n[1] || n[0]), (n[2] = n[2] || n[0]), (n[3] = n[3] || n[1]), n
      );
    }),
...
var headerIntroAnimation = anime
  .timeline({
    easing: "easeOutSine",
    autoplay: false,
  })
  ...
  function pause() {
    ...
    animation = anime
      ...
      .add(
        {
          targets: ".easing-visualizer .dot",
          translateY: anime.stagger(["-144px", "144px"], {
            easing: defaultEase,
            from: "last",
          }),
          duration: duration,
  ...
  function pause() {
    ...
    animation = anime
      ...
      .add({
        targets: ".cursor",
        translateX: {
          value: anime.stagger(-cell, { grid: grid, from: index, axis: "x" }),
        },
        translateY: {
          value: anime.stagger(-cell, { grid: grid, from: index, axis: "y" }),
        },
        scale: 1,
      })
      .add(
...
var timeControlAnimation = (function () {
  ...
  var animationPXOffset =
    (timeControlEl.offsetWidth - (timeControlEl.parentNode.offsetWidth - 20)) /
    2;
  if (animationPXOffset < 0) animationPXOffset = 0;

  function pxToTime(px) {
    var width =
      window.innerWidth > rullerEl.offsetWidth
  ...
  function animateProgress(el) {
    var animation = anime
      .timeline({
        targets: el,
        duration: function () {
          return anime.random(400, 1800);
        },
        easing: function () {
          return easings[anime.random(0, easings.length - 2)];
        },
        complete: function (anim) {
          animateProgress(anim.animatables[0].target);
        },
      })
      .add(
        {
...
var scrollToGettingStartedLink = document.querySelector(

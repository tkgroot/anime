...
function animePlayer(instance, containerEl) {
  ...
  function convertMStoEM(ms) {
    return ms / 100;
  }
  function convertEMtoMS(em) {
    return parseFloat(em) * 250;
  }
  ...
  function createAnimationLog(animObj, timelineOffset) {
    ...
    anim.player.animationEl = createEl(
      "div",
      "ap-animation",
      anim.player.trackEl
    );
    anim.player.delayEl = createEl("div", "ap-delay", anim.player.animationEl);
    anim.player.endDelayEl = createEl(
      "div",
      "ap-delay",
      anim.player.animationEl
    );
    ...
    anim.update = function () {
      ...
      anime.setValue(anim.player.delayEl, {
        width: (anim.delay / anim.duration) * 100 + "%",
      });
      anime.setValue(anim.player.endDelayEl, {
        width: (anim.endDelay / anim.duration) * 100 + "%",
      });
    };
